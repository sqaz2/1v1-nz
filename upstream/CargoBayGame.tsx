...
export default function CargoBayGame({ onClose }: CargoBayGameProps) {
  ...
  const pull = (): string => {
    ...
    if (!d.bag || d.bag.length === 0) d.bag = shuffleBag();
    return d.bag.pop()!;
  };
  ...
  const startGame = () => {
    ...
    const tick = (now: number) => {
      ...
      const dt = now - gRef.current.prev2;
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'stretch', height: '100%' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5, width: 65, flexShrink: 0 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <button style={ctrlBtnStyle} {...btnHandlers('L', moveLeft, true)}>◀</button>
          <button style={ctrlBtnStyle} {...btnHandlers('D', doSoftDrop, true)}>▼</button>
          <button style={ctrlBtnStyle} {...btnHandlers('R', moveRight, true)}>▶</button>
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          <button
